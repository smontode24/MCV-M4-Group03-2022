...
def fundamental_ransac_estimation(matches, points1, points2):
    ...
    F, indices_inlier_matches = fundamental_matrix_ransac(points1, points2, 1, 5000)
...
def normalize(x):
    ...
    mu, std = np.mean(x, 1), np.std(x)
    std = np.sqrt(2)/std
    Tr = np.array([[std, 0, -std*mu[0]], [0, std, -std*mu[1]], [0, 0, 1]])
    xt = Tr @ x
...
def fundamental_matrix(points1, points2):
    ...
    T1, points1n = normalize(points1)
    T2, points2n = normalize(points2)
...
def inliers_indices(F, x1, x2, threshold):
    """
    Find indices of inlier points given a Fundamental Matrix F
    ...
    inliers_indices = np.where(d[0,:] < threshold)
    
    return inliers_indices[0]

def fundamental_matrix_ransac(points1, points2, threshold, max_iterations):
    """
    Estimate the Fundamental matrix using RANSAC and a set of matching points between two images.
    :param points1: Points of image 1
    :param points2: Points of image 2
    :param threshold: Distance threshold to consider points as inliers for an estimated F
    :param max_iterations: Worst case number of maximum iterations
    :return: F: Fundamental matrix
             inliers: indices of inlier points for the estimated Fundamental Matrix
    """
    ...
    while it < max_it:
        ...
        inliers = inliers_indices(F, points1, points2, threshold)
    ...
    print(f"Robust estimation of Fundamental Matrix. Iter: {it} Num-Inliers: {inliers.shape[0]}")
    return F, inliers
